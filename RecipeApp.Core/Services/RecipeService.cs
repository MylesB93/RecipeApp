	...
	public class RecipeService : IRecipeService
	{
		...
		public async Task<Recipe> GetRecipeAsync(string id)
		{
			...
			if (response.IsSuccessStatusCode)
			{
				...
				return new Recipe() { Id = apiResult?.Id ?? "", Name = apiResult?.Name ?? "", Ingredients = apiResult?.Properties?.RecipeIngredients?.Select(i => new Ingredient { Name = i?.Name ?? "" }).ToList() ?? new List<Ingredient>(), Utensils = apiResult?.Properties?.RecipeUtensils?.Select(u => new Utensil { Name = u?.Name ?? "" })?.ToList() ?? new List<Utensil>(), Instructions = apiResult?.Properties?.CookingInstructions ?? Array.Empty<string>() };
			}
			else
			{
